...
def format_resume_as_html(resume_data: dict) -> str:
    ...
    if experiences:
        ...
        for exp in experiences:
            html_parts.append(f"<h3>{exp.get('role_title', '')} - {exp.get('company_name', '')}</h3>")
            html_parts.append(f"<p><em>{exp.get('start_date', '')} - {exp.get('end_date', 'Present')}</em></p>")
    ...
    if educations:
        ...
        for edu in educations:
            html_parts.append(f"<h3>{edu.get('degree', '')} in {edu.get('major', '')}</h3>")
            html_parts.append(f"<p>{edu.get('university_name', '')}</p>")
            if edu.get('start_date') or edu.get('end_date'):
                html_parts.append(f"<p><em>{edu.get('start_date', '')} - {edu.get('end_date', '')}</em></p>")
    ...
    if skills:
        ...
        for skill in skills:
            ...
            html_parts.append(f"<li>{skill_name}" + (f" ({skill_type})" if skill_type else "") + "</li>")
        html_parts.append("</ul>")
    ...
    if languages:
        ...
        for lang in languages:
            html_parts.append(f"<li>{lang.get('language', '')} - {lang.get('proficiency_level', '')}</li>")
        html_parts.append("</ul>")
    
    return "\n".join(html_parts)

def render_cv_comparison(enhance_data):
    # Local styles to keep the change report aligned and scrollable
    st.markdown(
        """
        <style>
        .change-report-box {
        ...
        /* Keep CV panel height aligned with change-report and scroll when longer */
    ...
    st.markdown('<div class="comparison-badge">📄 Personalized CV Preview</div>', unsafe_allow_html=True)
    ...
    # Show the personalized CV with a side change-report summary box
    # `report_summary` is a free-text summary (string), not a list
    ...
    with col_report:
        ...
        if report_summary:
            # Render the free-text summary as a single block of text
            st.markdown(
                f"""
    ...
    st.markdown("---")
    st.markdown("### 📊 Personalization Summary")
    ...
    # # Keep detailed view in an expander for deeper inspection
    # change_report = enhance_data.get('change_report', [])
    # if change_report:
    #     with st.expander("📝 View chages Summary"):
    #         for change in change_report:
    #             st.markdown(f"**{change.get('field_or_location', 'Change')}**")
